package me.aviloo.myclans.menuSystem.paginatedMenu;

import com.tcoded.folialib.FoliaLib;
import com.tcoded.folialib.wrapper.WrappedTask;
import me.aviloo.myclans.MyClans;
import me.aviloo.myclans.menuSystem.PaginatedMenu;
import me.aviloo.myclans.menuSystem.PlayerMenuUtility;
import me.aviloo.myclans.menuSystem.menu.ClanJoinRequestMenu;
...
public class ClanListGUI extends PaginatedMenu {
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        if (event.getCurrentItem().getType().equals(Material.PLAYER_HEAD)){
            ...
            UUID target = UUID.fromString(event.getCurrentItem().getItemMeta().getPersistentDataContainer().get(new NamespacedKey(MyClans.getPlugin(), "uuid"), PersistentDataType.STRING));
            ...
            PlayerMenuUtility playerMenuUtility = MyClans.getPlayerMenuUtility(player);
            playerMenuUtility.setOfflineClanOwner(Bukkit.getOfflinePlayer(target));
            new ClanJoinRequestMenu(MyClans.getPlayerMenuUtility(player)).open();
        ...
        }else if (event.getCurrentItem().getType().equals(Material.SPECTRAL_ARROW)) {
            ...
            player.playSound(player.getLocation(),Sound.UI_BUTTON_CLICK,5,0);
                    ...
                    if (clans != null && !clans.isEmpty()){
                        for (int i = 0; i < getMaxItemsPerPage(); i++){
                            ...
                            if (clans.get(index) != null) {
                                ...
                                meta.setLore(lore);
                                meta.getPersistentDataContainer().set(new NamespacedKey(MyClans.getPlugin(), "uuid"), PersistentDataType.STRING, clan.getClanOwner());
