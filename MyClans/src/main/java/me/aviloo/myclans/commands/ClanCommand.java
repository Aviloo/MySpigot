...
import me.aviloo.myclans.commands.clanSubCommands.*;
...
public class ClanCommand implements CommandExecutor {
    ...
    public static void updateBannedTagsList(){
        ...
        task1 = foliaLib.getImpl().runLaterAsync(() ->
                bannedTags = MyClans.getPlugin().getConfig().getStringList("clan-tags.disallowed-tags"), 1L, TimeUnit.SECONDS);
    }
        ...
        if (sender instanceof Player) {
            ...
            if (args.length < 1) {
                if (clansConfig.getBoolean("use-global-GUI-system")) {
                    new ClanListGUI(MyClans.getPlayerMenuUtility(player)).open();
                    return true;
                }
